...
public class DragListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		convertView = LayoutInflater.from(mContext).inflate(
				R.layout.list_item_group_management, null);

		initItemView(position, convertView);

		TextView titleTv = (TextView) convertView
				.findViewById(R.id.itemTitle_tv);
		...
		if (isChanged) {// 判断是否发生了改变
			...
			if (position == mInvisilePosition) {
				...
				if (!mShowItem) {// 在拖拽过程不允许显示的状态下，设置Item内容隐藏
					...
					convertView.findViewById(R.id.dragItemImage_imageView)
							.setVisibility(vis);
					convertView.findViewById(R.id.dragItemClose_linearLayout)
							.setVisibility(vis);
